...
写代码写的有点头晕。打算把剩余的功能一点点实现，比如问题 19 选 10，也就是把问题组功能利用好。设计思路是开一个问题组的类，里面加两个计数器（本组所有问题、本组已答问题），然后在创建 Question 对象的时候增加“所有问题”计数器，在提交时遍历所有答案并增加“已答问题”计数器（注意每个被评价人要分开遍历），最后检测这两个计数器符不符合预先设定的标准。

不过做这个的时候比较脑残，直接拿 AnswerStore 里的全部答案加起来然后就 bug 了，居然没想到直接加一个 revieweeId 的过滤条件就是了……

### 2017/04/29 - 2017/05/01

五一假期，可以好好肝路由器屏幕了。有一些坑：

* strncpy 如果源字符串太长，那么复制到 n 字节停止以后**不会自动在目标后加 0**（虽然说这是当然，已经写到头了，再写可能就 segfault 了，但还是有点方）（于是导致了设备型号显示成了 XXXCOMM A1，23333）
* char 在 ARM 里默认居然是 unsigned，但是标准确实没定义应该是哪种，都是编译器指定的. 这在最开始搞 MentoHUST v4 的时候出现过，现在又出现了居然又踩到了…… 这次是因为参数解析过程死循环出不来，gdb 强行打断还看不到栈的情况（最初还以为是哪里又写越界了），只能单步去看。最后发现 `if ((opt = getopt_long(....)) != -1` 总是不成立，char 型的 opt 是 255，才发现这个问题。把 opt 的定义改成 signed char 就好了。话说编译器不是应该在比较符号不匹配的时候提醒一下的么？明明 -Wall -Wextra 都开了就是没有……
* 交换机口 1 和 2 日常混淆 [二哈]
* signal 可以在任意时候打断程序运行，比如链表插到一半的时候，害怕。如果程序主循环是 poll 之类的话，可以用一个 signalfd 来监听信号，然后屏蔽掉默认信号处理程序，就可以解决这个问题（一开始觉得跟 poll 和 fd 搅和会很麻烦，但是写起来就还好诶）
* autotools 也没有想象中的那么可怕，automake 和 autoconf 两边的工具分清楚就不会觉得很纠结了。基本上只需要 autoscan 完了以后在输出文件里加一句 `AM_INIT_AUTOMAKE`，参数都不要，automake 这边就差不多稳了。最后生成的 config.h 如果没什么不通用的东西的话也不想支持路径自定义的话，不判断也没关系，不然你跟我说 `#undef HAVE_MALLOC` 我能怎么办？我也很绝望啊！
* autotools 默认不管非代码文件。需要安装和打包的话，要弄个新的 Makefile.am 定义个 `[dist_]XXXDIR_data` 结尾的变量把它们包含进去，这样就会安装了。有些文件夹不支持作为 _data 的目标的，写个新的目录定义就好了：
...
（但是小白比较多，看不懂帖子甚至还乱猜乱说，也很无奈）

也看到有人开始说烧 PA 的事了，我也再次去看了下这些（我觉得耸人听闻的）玩意（要出事早就一点信号都没了）。分析这块的时候可以结合 Android 内核源码里开放的 bcmdhd 来看，brcmfmac 和 bcmdhd 还是有几分相似的。然而两边确实都没有 4366 的 PA 和 LNA 开关，有开关的全是 SoftMAC 的设备。那么就认为 PA 和 LNA 完全归芯片自己管好了，那么大的固件还要让主机控制 PA 岂不是很没面子？

> 先说结论：我无法确定驱动是否操作了 PA，也无法确定是否会损坏硬件，但**个人倾向**于没有影响（不会损坏硬件，无线信号不好可能是其他原因，官方固件继续信号不好先恢复出厂设置，重新开关各种无线设置，考虑是不是环境因素影响甚至心理作用，毕竟手机放那不动信号都能跳好几个 dBm）
>
> 理由：
>
> 1. brcmfmac 这个驱动支持很多型号的网卡（顺便纠正：BCM4366 不是 “唯一恰好开源” 的型号，能支持的型号还很多），比如软路由神卡和 R8000 里面用的 BCM43602. 要说会烧 PA 的话，R8000 应该首先就全挂了，然而并没有任何信息指明这一点。
>
> 注：brcmfmac 支持的芯片在[这里](https://wireless.wiki.kernel.org/en/users/drivers/brcm80211)看
>
> 1. 路由的 dhd.ko 在 Android 内核源码里有开源版本。虽然不确定是不是完全一样，但搜索关键字的话会发现两边的函数都差不多。在这个源码里搜索 PA、EPA、External PA 之类的关键字并不能看到除 BCM4331 以外的芯片的 PA 控制逻辑（而且在 BCM4331 上进行操作还不是为了提升功率，而是解决 SROM 读取的问题（我也不知道是啥问题））
> 2. 在各种路由器自带的 dhd.ko 这个闭源驱动二进制文件的 readelf 结果里搜索上述关键字，也不能发现任何有关的匹配，唯一存在的函数也就是跟 dhd.ko 开源代码和 brcmfmac 中一样的对 BCM4331 中的 PA 的操作。分析调用流程也能发现如果卡是 BCM4366 那就不会调用这个函数。虽然不能保证其他地方没有魔改，但既然 BCM4331 的 PA 操作单独拿出来做了一个函数，那么其他型号如果要操作 PA 的话也应该另开一个函数（而且表示驱动编写人员已经注意到 PA 的存在）。现在没有的话，我倾向于没有操作，BCM4366 的 PA 是由芯片自己控制的。
>
> （转自我的[回复](http://koolshare.cn/forum.php?mod=redirect&goto=findpost&ptid=99044&pid=996854)）

感觉好久没看电影了，有点想啥都不管了去看一场，但又想写代码 2333

学弟们都回家了，五一也没人玩，最后一个人去看了速8，感觉是第一次一个人看电影？手动三个二哈

（话说那个筛掉天眼重复数据时利用的随机数发生器漏洞，跟 Mifare Classic 的 nested authentication 是不是一样的原理？此外整片看着真的爽到爆炸爽到起飞还想再看一遍所以有没有人一起二刷啊23333）

### 2017/05/02

把用户体验弄得更和谐一点：把是否允许重复答题的设置发到客户端，这样在答完以后退回列表时就能决定是不是要标记为已完成了。不然在允许重复答题时退回去发现标记为已完成，有点尴尬。然后按被评价人分开检查答案数目，这个客户端基本就完成了。

好像从这时候开始有周围的人要接手项目了，教学任务开始多了😂老师也回来了，给他看各种新加的功能，应该也满意了，感觉不知道下一步做什么……

路由屏幕方面，发现 `uci get network.wan.ifname` 返回的是底层设备名称，如果是 PPPoE 这样 IP 在虚拟接口上的，那就会认为没有连接网络了。改用 `ifstatus wan` 来看地址和接口比较好。

### 2017/05/03

今天导师说课题不变，论文还是按原题写，不过推送只要做出来就行，可以花大篇幅讲述后来做的这些内容…… 可以很强势，至少定下了方向

然而这几天教学任务越来越多了😂有时候还感觉有点懵逼，到底是开始写推送，还是接着整合其他人写的界面完善之前的项目？此外也不知道要用什么语言来写监测程序…… 还是用 Python？

有人开始研究无线网卡固件。我看了下内核源码，发现当时 R8000 帖子里提到的国家码转换功能确实是只有 R8000 有（在 of.c 里面）并且 US -> Q2, JP -> JP 的区域码映射看来是固定的。那么问题来了，regrev 怎么取得呢？找对应路由的 NVRAM dump？然而并不好找…… 随手谷歌了两个，填进去，Firmware rejected country code settings. 解包对应版本的原厂固件，暴力 grep 找到个数值，填进去，没 reject 了但是也没功率增强效果。进一步找到了强行在 brcmfmac 上用 txpwr_target_max 的办法，发现设定的功率并不能生效，想到可能是 override 的问题，于是手加 log 看到目前用的固件确实不允许 override（标志明明设上去了，但再读出来还是没有）于是就去试了下几个不同固件的功率 override，发现原厂固件、AC88U 的固件支持 override，功率开上去以后声音惊人。

好吧，输出功率问题算是结了。那么为什么明明美区支持 5G 高信道开 30dBm 功率，但功率限制还是 26dBm 呢？

### 2017/05/04

感觉没事做，顺手写了“下载自己上一次的答案”这个功能。然后就有点浪，开始看怎么把无线固件逆向出来。一开始以为是 VxWorks，因为 binwalk 在一个固件里发现了 VxWorks symbol table，但这好像是个 false positive，其他固件都没有这玩意。然后看网上说的打开 -I 选项，显示无效头部，也许能发现变种 LZMA. 试了下，并没有有效的 LZMA 头部，一字节都没有…… IDA 试了好几个地方都不能得到像样的代码，怀疑是像 CFE 一样部分加密了，于是放弃。（毕竟 FW Console 里面的字符串都找不到，这就很有问题了）

顺便，华为的 4.4 上为啥标题字体那么小，比正文还小？为啥小米的 6.0 通过 USB 安装还要另外打开这个功能，不然直接 USER_CANCELLED？特么的不知道 Android 本来有个“通过 USB 验证应用”？弹个框问是否安装能死？更别说最后即使开了也还是出于莫名原因失败。讲真黑国产手机不是没有道理。

### 2017/05/05

继续教学工作 [二哈] 实验室确实应该跟计算机那边合作完成项目，并且至少应该告诉下面的学生这里可能会以计算机为主，不然什么计算机方面的准备都没有，突然被通知接盘难道不会导致效率极其低下？

### 2017/05/06 - 07

周志完成约 20%. 要开始写论文目录了……

顺便记一下，如果黑苹果在没打 fcvo patch 的情况下就直接启动系统，可能会把无线网卡的寄存器写挂，不彻底断电关机重启的话无论再怎么改都过不去 wlcStart failed. 被坑了一个多小时……

### 2017/05/08

开始写微信推送啦。实验室的设备最初设计的时候可能没注意开发和生产隔离，内网环境只有生产服务器而没有开发服务器，所以不能随便进去写代码（最好就别打生产服务器的主意），风险太大。此外，现在有带穿透的 web 接口可用，所以实际上在外网用已有的接口轮询跟在内网监视数据库没啥太大区别。（顺便，听学长说隔离通信程序的原理是扫描一个固定的文件夹，有新文件就发出去，然后把文件删掉。Excuse me？我还以为有个 control socket 或者至少有个 signal handler 什么的，别人要发东西的时候通知它路径，让它去发，结果是轮询的。这要实现信号啥的总比穿透隔离推送数据变化简单多了吧？）

接口很好找，F12 就好。通过学姐了解到系统是十五分钟自动刷新一次数据，所以我也不用刷得太勤快，每五分钟检查一下接口就好了。（再顺便，穿个隔离，查询速度从几百毫秒暴涨到十秒，真的没问题吗？）

具体设计方面我觉得 shell 都能写…… 记录上一次查询的时间，然后向接口发起上次查询到现在中的报警，只要有返回就推送。我先写麻烦点的，跟微信对接的部分吧。

时间紧迫（可以再看看这一节的标题，谢谢各位），不自己重做微信公众平台的轮子了，直接从 GitHub 找了一个叫做 wechatpy 的库（链接稍后补上），看起来功能很全，很不错。

时间紧迫 x2，没时间去申请正儿八经的微信服务号来开启主动下发和模板消息权限。直接用接口测试号来做。它的限制是不能改名，最多 100 人关注，这对于这样的测试而言够用了。

照着 wechatpy 的 Flask 示例做个服务器配置校验的功能，用 ssh 穿透到 VPS 上（不得不说简单的穿透让 ssh 来做比 frp / ngrok 简单多了），一次成功，棒。想起了三年前第一次自己用 PHP 写的时候被 U+FEFF 坑三天不能验证服务器配置的黑历史。然后写简单的消息处理，从 ping / pong 开始，到解析简单的关联工号和取消关联指令、加入 Django 模型操作，把用户的 OpenID 与系统中的用户模型关联起来，然后就可以通过预先给每个用户划分的组（从现有的 ERP 系统导入进来的）群发消息了。同时，为了在消息里把后端接口里的设备 ID 转化为可读的名称，还要加入电站和机组的模型，把名称和后端返回的 ID 关联起来。机组是隶属于电站的，这在后期设置用户归属的时候可能会方便查找（当然也符合最终用户的直觉——废话）。

有一点比较微妙的地方：微信公众平台的接口是往你的同一个网址上 GET 或者 POST，而一般自己做 API 的时候会做成像 Django admin 的 URL 一样 `/message/send` 的形式，看起来比较清晰。微信的做法算是降低了实现的复杂度吧。

构想完成，今天把模型、用户发消息绑定与解绑做好就差不多了。

BTW Android 6.0 上代码添加的 RadioButton MATCH_PARENT 真的占用全部宽度可能是 Android 官方的 bug，今天拿来一堆设备里 Android 6.0 的全部都有这个问题…… workaround 一发，对于低版本设备改成 WRAP_CONTENT 好了。老师说这样又太窄了，我能怎么办我也很绝望啊，难道把 Android 7.0 里面的 LinearLayout 自己拿来编译进去？忽然想起来有个 SupportLibrary 里有个 LinearLayoutCompat，或许能拿来用，但有没有 RadioGroupCompat 呢——没有 [doge] 所以还是要复制一堆代码进来……

### 2017/05/09

嗯。出差。这次是 boss 最终确认。再次测试，修了一些 bug，比如给 setText 传入了本来应该是 String.valueOf() 的 int，还有循环删子 view 时把循环终止条件写成 `== getChildCount()` 导致删不完这样的很傻 x 的问题…… 可能是写这些代码的时候脑子有点昏。

老师还是觉得 Android 6.0 上 WRAP_CONTENT 太挤了，于是想个办法，先水平排，检测到子 view 太小（暂定小于 48dp，我自己手机上显示 6 个 view 是这么多）的时候就切换到纵向。

这个功能说起来简单，实现起来还有点坑（对于我这样的彩笔而言）：

一开始的实现方式是在刚 containerView.addView() 后读 radioButton.getWidth()，写的时候就在想这时候还没绘制，应该读不到尺寸吧。事实证明确实读不到，全 0.

然后改成 radioGroup.post() 这里面读。乍看起来是可以的，但有个问题：一旦横改竖，那么在下次重用重新判断空间是否足够的时候就会判断成空间太多，于是又改回横的。

改成在 radioGroup.post() 里读 RadioGroup 的宽度，除以子元素数量，得到每个元素在横向时的大概宽度。这样还不错。

一直改到晚上预先 demo 才弄好。

### 2017/05/11

上午给分厂的人先 demo，上手体验以后得到如下这些反馈：

* 提交按钮固定在顶部
* 做笔记本电脑的负载测试
* 在不退出程序的情况下刷新问卷（比如修改允许重复提交啥的能立即生效）
* 给后端写个用户手册（嗯……）
* 自定义 CSV 输出格式（你猜我有时间写没有？）
* 做加权平均值这样的简单统计（同上）
* 问题排序，把后加的问题顶到上面去（我也想啊）
* 【重点】支持按题目分组显示，也就是横向滑动切换题目，一屏幕上是多个候选人的回答框。这样可以提供同一题目不同人的对比。
* 有一台设备突然掉线，然后提示 login first. （黑人问号，我的 admin session 一天不登录都不会过期啊，抓个包也看到过期是十天多，这是怎么玩过期了的…… 难道是系统自身的 cookie manager 挂了？）
* 某些设备上重用视图的时候还是有打分题纵横来回跳的问题
* 有些设备在键盘收起后 Activity 不会自动 resize，下面空一截（怪我咯）
* 提交时提示”提交后不能更改“
* 打开程序时自己选择自己属于哪个组（1. 这么信任下面的人的吗 2. 早不说要求，现在设计成在项目里给分组指定权重，程序刚开还没项目呢怎么知道有哪些组可用）
* 打开程序时显示每个项目特定的通知，比如本次测评结果将保密 blahblah. （同上 2.）
* 改应用名字
* 隐藏问卷列表上的说明文字，因为没啥用
* 每提交一个问卷就生成一个 PDF 做存档（我怀疑你没听说过数字签名，说这个行业都是这么做的，你怎么不知道群晖的 Surveillance Station 的录像是通过数字水印来保证法律上可用作证据的呢）

中午给了一小时时间，能改多少改多少 [二哈] 你猜我能改多少？

中午吃饭的时候还提到说要注意细节设计，”提交“和”帮助“两个按钮都是文字，太占地方了。那我以前用对勾的时候是谁说用户看不懂对勾的意思要改成文字的？

下午最终 demo 倒是还好，然后又有以下新要求：

* BUG: 项目通知那里如果没点确认就会卡死在登录界面
* 部分问卷按问题分组，部分问卷按人分组（老实说这在一开始做出来的时候怎么不提）
* 提交后立即清除答案（上次演示的时候 boss 还明确说希望提交后看到答案的，这次说”要揣摩上司的意思“。这样正面要求相反的问题我反正是猜不到答案，毕竟我又不是在这个行业做了很久能知道这里面的常见规则）

反正是通过了，回去约基友。

Lynx 是说很巧，早两天来他还没回来，晚一天来他就回家了。讲真，见到了本以为再也见不到的同学是一件很神奇的事，而且还是在大家都年轻的时间。（虽然心里很激动，但是言语和行动上却不知道如何表达，唉……）（甚至还把某条消息发错人了，一下子意思就变了，百分尴尬……）

### 2017/05/10

一早五点半起来赶车，最后差一分钟没赶上，在 KFC 坐了一上午，继续解 bug 和需求，比如提交后清除答案，也不要从网络加载答案（于是我两边就都白写了）

思考了一下纵横交替重用时布局错乱的问题，发现原来的逻辑有问题。原来的逻辑是：

如果子元素宽度小于 48dp，且是横排，就转为竖排；

其次，如果子元素宽度大于 96dp，且是竖排，就转为横排。

这么设计是为了在跟普通单选题重用时能够减少纵横切换的闪烁。（突然觉得这讲不通，一点道理都没有）于是改成第一条判断失败且是竖排的话，就都改成横排。这样没有中间那道 48~96dp 的”保持区间“，就统一了。

然后写了一点自动重新 deviceLogin 的方法，抽离合并了全部的 AsyncTask，可以解决突然 cookie 失效的问题。原因未知的问题真麻烦……

等回到学校都六点多了。随便吃点啥算了。（高铁上的饭吃完了肚子疼？好久都没肚子疼过了……）

### 2017/05/12

说是周六了，但下周要用，所以还有一些细节问题要解决：

* 加一个退出（并清除数据）的按钮（实际上应该在退出主 Activity 的时候就清除的，然而总是有人觉得没有叉叉不安全，老实说你们所认为的用户们真的不知道按返回吗？）
* 上传答案失败时重新显示提交按钮

然后把按问题分组和自动重新登录都做完了。跟一个对外观有要求的学弟一起改了下前者的布局，现在能看了。

### 2017/05/13 - 14

会长的想法和自己的不谋而合，给随手实现成网站了，居然有 2000 的日 UV，技术部大家共同创造的奇迹~ 说说从协会群体转回了实验室和电气专业群体，6666

### 2017/05/15

本来要去北京的，大 boss 说论文写不完，老师就一个人去了。然而要做压力测试的数据，还要给一台空着的电脑装这套系统，最后也没能开始做自己的毕设。为什么就要用 Windows 做服务端？压力测试结果那么难看，Django 又不自带 FastCGI 服务端……

到了晚上老师说问题太多搞不定，于是还得去。

在车站等检票的时候突然脑子一沉？？？好像经历过一两次这个感觉，不太妙啊。到底是补周记还是写论文还是睡觉呢……