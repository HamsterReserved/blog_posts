...
EA9500 的固件在 K3 上用会导致 LEDE 自己开的 WiFi 能够被搜到却无法连接：一旦 associate 完成，客户端发第一个数据帧（或者别的 Class 3 的帧），路由器就会发 Deauth w/ reason = 7 踢掉客户端。但如果把系统的配置文件拷出来，手动启动 hostapd 就不会有问题。如果把手动的 hostapd 干掉，再让系统开，也一样不行。开了 brcmfmac 的调试，看不出手动和自动启动的 hostapd 有什么问题。即使是把 /lib/netifd/wireless/mac80211.sh 里全部有问题的 iw 调用都去掉也不行。

R8500 的固件就没有这个问题，系统开的可以连，也能显示已关联客户端列表。但是客户端当前使用的信道宽度不能显示，总是 5MHz。不过能用也无所谓了。

继续检查发现 EA9500 的固件会让 LEDE 显示出 160MHz 的带宽选项，但开不起来。auto 频段会卡在 ACS failed，指定频段会卡在 HT_SCAN failed（似乎是 HT_SCAN），80MHz就没有问题了。感觉是扫描也不太正常？EA9500 的固件版本字符串有 -dyn160，R8500 没有。R8500 的固件也不能在系统显示出 160MHz 的带宽。此外，EA9500 的固件是 10.10.122.x，R8500 和 K3 原本的是 10.10.69.x，可能兼容性与这个也有关系。
